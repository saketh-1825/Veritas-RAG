        ...
        <div className="flex flex-col gap-1.5 bg-slate-900/40 p-2.5 rounded-xl border border-slate-900/80">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${backendStatus?.status?.includes('online') || backendStatus?.status?.includes('healthy') ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-amber-400 shadow-amber-400/50'} shadow-sm`} />
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                {backendStatus?.status || 'UI Prototype'}
              </span>
            </div>
            <span className="text-[9px] text-cyan-400 font-semibold">
              {backendStatus?.version || 'v0.2.0-preview'}
            </span>
          </div>
        </div>
      </div>
